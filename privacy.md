...
- Shows **local notifications** for the sleep timer — a reminder right when it's set to turn off your TV, and a heads-up if it couldn't reach your TV or the timer was missed while your phone was asleep. These notifications are generated and delivered entirely on your phone by the OS; nothing about your TV, your usage, or the notification's content is sent to us or to any third party. On Android, if you turn on the optional **"TV controls in the notification shade"** setting (off by default), OneMote keeps a small background service running so it can show Power/Volume/Mute/Play-Pause buttons in an ongoing notification and, when a sleep timer is armed, so the timer can still turn your TV off even while your phone is locked. That service only ever talks to your TV over your home WiFi, exactly like the rest of the app — it does not send anything off your phone, and you can turn it off in Settings at any time.
...
| Notifications | Show the sleep-timer reminder/outcome, and — only if you turn it on — the optional TV-controls notification (Android); requested only when you first use one of these, never at launch |
| Background service (Android, optional) | Keeps the sleep timer running, and the optional notification-shade TV controls available, while your phone is locked or the app isn't open — talks only to your TV over your WiFi |
